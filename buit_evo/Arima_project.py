...
# 导入数据
data = pd.read_excel(r'/Users/luoliang/Desktop/项目开发文件/新销量预测文件/ARIMA/ARIMA预测文件.xlsx',sheet_name =5,index_col='日期')
data_train = data.loc[:'2022-03-31']
data_test = data.loc['2022-04-01':]
# 数据清洗
std_data = data_train['销量'].std()
avg_data = data_train['销量'].mean()
data_train['z_score'] = (data_train['销量']-avg_data)/std_data
...
def prediction(data):
    model = sm.tsa.ARIMA(data,order =(7,1,10)).fit()
    pre = model.forecast(62)
